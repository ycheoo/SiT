...
def init_distributed_mode(args):
    ...
    print()
    print(
        "| distributed init (rank {}), gpu {}".format(args.rank, args.gpu), flush=True
    )
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    ...
    if loss_scaler is not None:
        checkpoint_paths = [result_dir / ("checkpoint-%s.pth" % epoch_name)]
...
def all_concatenate(tensor):
    world_size = get_world_size()
    output_tensors = [tensor.clone() for _ in range(world_size)]
    dist.all_gather(output_tensors, tensor)
    concatenated_tensor = torch.cat(output_tensors, dim=0)
...
def save_results(save_name, results, log_writer, result_dir):
    if result_dir and is_main_process():
